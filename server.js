...
async function handleChoreSchedule(req, res) {
  ...
  const dailyMax = Math.max(1, Math.min(12, Number(payload.dailyMax) || 2));
  const targetPerPerson = Math.max(1, Math.min(140, Number(payload.targetPerPerson) || 35));
  ...
  const systemPrompt = [
    ...
    "Use only userId values from the provided members list. Do not invent userIds.",
    "",
    "Output strict JSON only:",
    "{\"assignments\":[{\"userId\":\"string\",\"userName\":\"string\",\"memberIndex\":0,\"choreTitle\":\"string\",\"scheduledDate\":\"YYYY-MM-DD\",\"difficulty\":1,\"estimatedMinutes\":20,\"category\":\"string\",\"ageMin\":0}]}",
