...
const el = {
  ...
  choreCatalogForm: document.getElementById("choreCatalogForm"),
  choreCatalogList: document.getElementById("choreCatalogList"),
  choreAutoAssignMembers: document.getElementById("choreAutoAssignMembers"),
  choreTitleInput: document.getElementById("choreTitleInput"),
  choreFrequencyInput: document.getElementById("choreFrequencyInput"),
  choreDifficultyInput: document.getElementById("choreDifficultyInput"),
  choreMinutesInput: document.getElementById("choreMinutesInput"),
...
function normalizeChore(c) {
  const difficulty = Math.max(1, Math.min(3, Number(c.difficulty) || (c.frequency === "Monthly" ? 3 : c.frequency === "Weekly" ? 2 : 1)));
  return {
    id: c.id || uid(),
    title: c.title || "",
    ...
    difficulty,
    estimatedMinutes: Number(c.estimatedMinutes) || (difficulty === 3 ? 45 : difficulty === 2 ? 30 : 20),
    ageMin: Math.max(5, Math.min(18, Number(c.ageMin) || 8)),
    category: c.category || String(c.title || "").split(" ")[0] || "general",
    active: c.active !== false,
    autoAssignMemberIds: Array.isArray(c.autoAssignMemberIds) ? c.autoAssignMemberIds : [],
  };
}
...
function choreTemplates() {
  ...
  for (const c of state.chores) {
    ...
    const tpl = map.get(key) || {
      ...
      difficulty,
      estimatedMinutes: Number(c.estimatedMinutes) || (difficulty === 3 ? 45 : difficulty === 2 ? 30 : 20),
      ageMin: Math.max(5, Math.min(18, Number(c.ageMin) || (difficulty === 1 ? 6 : (difficulty === 2 ? 9 : 12)))),
      category: c.category || key.split(" ")[0] || "general",
      active: c.active !== false,
      autoAssignMemberIds: Array.isArray(c.autoAssignMemberIds) ? c.autoAssignMemberIds : [],
    };
    tpl.active = c.active !== false;
    tpl.autoAssignMemberIds = Array.isArray(c.autoAssignMemberIds) ? c.autoAssignMemberIds : tpl.autoAssignMemberIds;
    tpl.ageMin = Math.max(5, Math.min(18, Number(tpl.ageMin) || 8));
    map.set(key, tpl);
  }
...
function memberEligibleForTemplate(member, tpl) {
  const ageOk = Number(member.age || 0) >= Number(tpl.ageMin || 0);
  if (!ageOk) return false;
  const targets = Array.isArray(tpl.autoAssignMemberIds) ? tpl.autoAssignMemberIds.filter(Boolean) : [];
  if (!targets.length) return true;
  return targets.includes(member.id);
}
...
function renderChoreMemberOptions() {
  if (!el.choreAutoAssignMembers) return;
  el.choreAutoAssignMembers.innerHTML = state.members.map((m) => `
    <label class="checkbox-row">
      <input type="checkbox" value="${m.id}" data-chore-auto-member checked />
      <span>${escapeHtml(m.name)}</span>
    </label>
  `).join("");
}

function renderChoreCatalog() {
  if (!el.choreCatalogList) return;
  const rows = state.chores.slice().sort((a, b) => String(a.title || "").localeCompare(String(b.title || "")));
  el.choreCatalogList.innerHTML = rows.map((c) => {
    const targets = (Array.isArray(c.autoAssignMemberIds) ? c.autoAssignMemberIds : [])
      .map((id) => findMember(id)?.name)
      .filter(Boolean);
    const targetText = targets.length ? targets.join(", ") : "All members";
    return `
      <li>
        <div class="list-content">
          <strong>${escapeHtml(c.title)}</strong>
          <small>${escapeHtml(c.frequency)} · diff ${Number(c.difficulty) || 1} · ${Number(c.estimatedMinutes) || 20} min · min age ${Number(c.ageMin) || 8}</small>
          <small>Category: ${escapeHtml(c.category || "general")} · Auto-assign: ${escapeHtml(targetText)} · ${c.active ? "Enabled" : "Disabled"}</small>
        </div>
        <div class="row-actions">
          <button class="btn btn-secondary btn-sm" data-toggle-catalog-chore="${c.id}" type="button">${c.active ? "Disable" : "Enable"}</button>
          <button class="btn btn-secondary btn-sm" data-edit-catalog-chore="${c.id}" type="button">Edit</button>
          <button class="icon-btn" data-delete-catalog-chore="${c.id}" title="Delete chore">✕</button>
        </div>
      </li>
    `;
...
document.getElementById("memberForm").addEventListener("submit", async (e) => {
  ...
  renderInvolvedChecklist();
  renderMemberList();
  renderChoreMemberOptions();
  renderChoreCatalog();
});

el.choreCatalogForm?.addEventListener("submit", async (e) => {
  e.preventDefault();
  const title = String(el.choreTitleInput?.value || "").trim();
  if (!title) return;
  const frequency = String(el.choreFrequencyInput?.value || "Daily");
  const difficulty = Math.max(1, Math.min(3, Number(el.choreDifficultyInput?.value) || 1));
  const estimatedMinutes = Math.max(5, Math.min(120, Number(el.choreMinutesInput?.value) || 20));
  const ageMin = Math.max(5, Math.min(18, Number(el.choreAgeMinInput?.value) || 8));
  const category = String(el.choreCategoryInput?.value || "").trim() || "general";
  const active = !!el.choreActiveInput?.checked;
  const autoAssignMemberIds = Array.from(document.querySelectorAll("[data-chore-auto-member]:checked")).map((x) => x.value);
  ...
  e.target.reset();
  if (el.choreDifficultyInput) el.choreDifficultyInput.value = "1";
  if (el.choreMinutesInput) el.choreMinutesInput.value = "20";
  if (el.choreAgeMinInput) el.choreAgeMinInput.value = "8";
  if (el.choreActiveInput) el.choreActiveInput.checked = true;
  renderChoreMemberOptions();
  await saveState();
  renderChoreCatalog();
});
...
el.memberList.addEventListener("click", async (e) => {
  ...
  state.chores = state.chores
    .filter((x) => x.memberId !== deleteId)
    .map((x) => ({ ...x, autoAssignMemberIds: (x.autoAssignMemberIds || []).filter((id) => id !== deleteId) }));
...
el.choreCatalogList?.addEventListener("click", async (e) => {
  const toggleId = e.target.dataset.toggleCatalogChore;
  if (toggleId) {
    const chore = state.chores.find((x) => x.id === toggleId);
    if (!chore) return;
  ...
  const editId = e.target.dataset.editCatalogChore;
  if (editId) {
    const chore = state.chores.find((x) => x.id === editId);
    if (!chore) return;
    const title = prompt("Chore title:", chore.title || "") || chore.title;
    const frequency = prompt("Frequency (Daily/Weekly/Monthly):", chore.frequency || "Daily") || chore.frequency;
    const difficulty = Number(prompt("Difficulty (1-3):", String(chore.difficulty || 1)) || chore.difficulty || 1);
    const estimatedMinutes = Number(prompt("Minutes:", String(chore.estimatedMinutes || 20)) || chore.estimatedMinutes || 20);
    const ageMin = Number(prompt("Minimum age:", String(chore.ageMin || 8)) || chore.ageMin || 8);
    const category = prompt("Category:", chore.category || "general") || chore.category || "general";
    const memberNames = prompt("Auto-assign to (comma-separated names, blank = all):", (chore.autoAssignMemberIds || []).map((id) => findMember(id)?.name).filter(Boolean).join(", ")) ?? "";

    chore.title = String(title || chore.title).trim() || chore.title;
    chore.frequency = ["Daily", "Weekly", "Monthly"].includes(String(frequency || "")) ? frequency : "Daily";
    chore.difficulty = Math.max(1, Math.min(3, Number(difficulty) || 1));
    chore.estimatedMinutes = Math.max(5, Math.min(120, Number(estimatedMinutes) || 20));
    chore.ageMin = Math.max(5, Math.min(18, Number(ageMin) || 8));
    chore.category = String(category || "general").trim() || "general";
    const targets = String(memberNames || "")
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean)
      .map((name) => findMemberByLooseName(name)?.id)
      .filter(Boolean);
    chore.autoAssignMemberIds = targets;
    await saveState();
    renderChoreCatalog();
    return;
  }

  const deleteId = e.target.dataset.deleteCatalogChore;
  if (!deleteId) return;
  state.chores = state.chores.filter((x) => x.id !== deleteId);
  await saveState();
  renderChoreCatalog();
});
